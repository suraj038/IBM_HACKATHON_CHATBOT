    ...
    */
    public function processChatInput($p_chat_text, $p_watson_api_context)
    {
      ...
      if(isset($watson_reply_msg['watson_api_context']))
      {
         ...
         if(isset($watson_current_context->action) && strtoupper($watson_current_context->action) == 'ACTION_MAKE_RESERVATION' && isset($watson_current_context->i_name))
         {
            $i_name = empty($watson_current_context->i_name) ? null : trim($watson_current_context->i_name);
            $i_address = empty($watson_current_context->i_address) ? null : trim($watson_current_context->i_address);
            $i_phn_no = empty($watson_current_context->i_phn_no) ? null : trim($watson_current_context->i_phn_no);
            $i_age = empty($watson_current_context->i_age) ? null : trim($watson_current_context->i_age);
            $account_type = empty($watson_current_context->account_type) ? null : trim($watson_current_context->account_type);
            


            $reservationObj = new Reservation();
            $return_array = $reservationObj->createReservation('CREATE', array('i_name' => $i_name, 'i_address' => $i_address, 'i_phn_no'=> $i_phn_no, 'i_age'=>$i_age, 'account_type'=>$account_type, 'r_status'=>'CONFIRMED'));
            
            //Remove the input parameters from context
            if(isset($watson_current_context->i_name)) 
               unset($watson_current_context->i_name);
            if(isset($watson_current_context->i_address)) 
               unset($watson_current_context->i_address);
            if(isset($watson_current_context->i_phn_no)) 
               unset($watson_current_context->i_phn_no);
            if(isset($watson_current_context->i_age)) 
               unset($watson_current_context->i_age);
            if(isset($watson_current_context->account_type)) 
               unset($watson_current_context->account_type);
         }
         ...
         if(isset($watson_current_context->action) && strtoupper($watson_current_context->action) == 'ACTION_CANCEL_RESERVATION' && isset($watson_current_context->i_reservation_number))
         {
            ...
            {
               $return_array = $reservationObj->createReservation('UPDATE', array('id'=>$reservation_data['id'],'i_name' => $reservation_data['i_name'], 'i_address' => $reservation_data['i_address'], 'r_status'=>'CANCELLED'));
               
               if(isset($return_array['status']) && $return_array['status'] == 'S')
               {
                  $return_array['status'] = 'S';
                  $return_array['message'] = 'Your request to Block Account number <b>' . $reservation_data['id'] . '</b> is taken care.';
               }
               else
               {
                  $return_array['status'] = 'E';
                  $return_array['message'] = 'Sorry. Some system error happened while cancelling Account number <b>' . $reservation_data['id'] . '</b>.';
               }
            }
            else
            {
               $return_array['status'] = 'E';
               $return_array['message'] = 'Sorry, either the entered Account number is not found or it is already cancelled.';
            }
         ...
         if(isset($watson_current_context->action) && strtoupper($watson_current_context->action) == 'ACTION_RESCHEDULE_RESERVATION' && isset($watson_current_context->i_reservation_number))
         {
            ...
            if(isset($reservation_data['r_status']) && $reservation_data['r_status'] != 'CANCELLED')
            {
               $return_array = $reservationObj->createReservation('UPDATE', array('id'=>$reservation_data['id'],'i_name' => $i_name, 'i_address' => $i_address, 'r_status'=>'RESCHEDULED'));
               
               if(isset($return_array['status']) && $return_array['status'] == 'S')
               {
                  $return_array['status'] = 'S';
                  $return_array['message'] = 'Your reservation number <b>' . $reservation_data['id'] . '</b> is updated with date: ' . $i_name . ' time: ' . $i_address;
               }
               else
               {
                  $return_array['status'] = 'E';
